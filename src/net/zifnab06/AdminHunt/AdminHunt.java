...
public class AdminHunt extends JavaPlugin implements Listener {
    ...
    public void onEnable() {
        ...
        log("AdminHunt has been started successfully.");
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String name, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("adminhunt-list") && sender instanceof Player) {
            if (sender.hasPermission("adminhunt.toggle.list")) {
                String playerList = "Enabled players: " + (ENABLED_PLAYERS.isEmpty() ? "none!" :
    ...
    private void toggleHuntedStatus(Player player) {
        ...
        if (!isActive(player)) {
            getServer().broadcastMessage(ChatColor.GREEN + "[AdminHunt] An admin hunt has begun! Find (and " +
												 "kill) " + player.getName() + " as quickly as you can!" + ChatColor.RESET);
            ENABLED_PLAYERS.add(uuid);
        } else {
